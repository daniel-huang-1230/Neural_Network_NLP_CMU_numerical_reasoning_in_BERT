...
# ****** New method - Added by Eason ******
def getThreshold(W, p = 0.4):
    return np.quantile(np.abs(W.clone().detach().cpu().numpy()), q = p, interpolation="lower")
...
def prunning(model, prune_layer_name = "!!", prune_rate = 0):
    layer_names = [
      "attention.self.query",
      "attention.self.key",
      "attention.self.value",
    ...
    for name, param in model.named_parameters():
        # g += param.numel()
        for prune_layer_name2, cut_ratio in zip(layer_names, ratios):
            if prune_layer_name2 in name and prune_layer_name2 != "output.dense":
                num_pruned += (param.numel() * cut_ratio)
                # print(name)
                param.data = (param.data * (torch.abs(param.data) > getThreshold(param, p = cut_ratio)).float())
                continue
            ...
            if prune_layer_name2 in name and prune_layer_name2 == "output.dense":
                if "attention" not in name:
                    ...
                    # print(name)
                    param.data = (param.data * (torch.abs(param.data) > getThreshold(param, p = cut_ratio)).float())
    # ratio = num_pruned * prune_rate / 85017600  114868606
    ratio = num_pruned / 85017600
    # print(g)
    print("Global Sparsity: {}%".format(ratio * 100))


      # if isIn:
      #     print(name)
      #     # print(param.shape)
      #     param.data = (param.data * (torch.abs(param.data) > getThreshold(param, p = prune_rate)).float())
      #     # print((torch.abs(param) > getThreshold(param, p = 0.5)).float())

# def prunning(model, prune_rate = 0):
#   for name, param in model.named_parameters():
...
#                         help="Whether to tokenize numbers as digits.")
    ...
    # ****** New - Added by Eason ******                         
    parser.add_argument('--prune_rate', type=float, default=0, help="The precentage of weights to prune")
    ...
    prune_layer_name = args.prune_layer_name
    # print(getSparsity(model))  # 85017600
    if prune_rate == 0:
      print("$$$$$$$$$$$$$$$$$$$$$$")
      print("$$$ No Prunning $$$")
      print("$$$$$$$$$$$$$$$$$$$$$$")
      ...
      print("********************************************")
      print("*** Start Prunning Model, rate: {} ***".format(prune_rate))
      print("********************************************")
      prunning(model, prune_layer_name, prune_rate)
